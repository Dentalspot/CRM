...
**Active feature**: — entre ciclos (2026-04-22 PM). **Spec 022 MVP Lean DONE + Phase E DONE** — App funcional para cobros con enforcement de límites activo en UI. Phases A+B+C+D+E+G completas. Phase E se cerró en la sesión PM: hook `useActivePlanLimits` + guards en PatientModal, NewAppointmentForm, InviteTherapistModal que invocan RPC `check_plan_limit` y muestran `UpgradeModal` al exceder límite. Además cleanup branding FonoKit visible residual (5b) + regeneración `supabase/schema.sql` a PG 17.9 (se upgradó server de 15.8→17.6). Siguiente paso: **launch beta con 5-10 dentistas usando cupón BETA-3M-2026** (100% off, 3 meses renovación) + monitoring manual. Follow-ups pendientes por prioridad: (1) meta-spec `fix-mercadopago-critical-bugs` P0 antes de scale a 100+ users (16-22h, F-001/F-002/F-003/F-005), (2) Phase F smoke E2E completo con usuarios beta reales (1-2h), (3) E3 smoke manual del enforcement (crear 6° paciente en Free → modal, aplicar cupón, reintentar), (4) `create-clinical-boxes-table-and-enforcement` cuando se diseñe UX sillones (box siempre retorna true en RPC por ahora). Ver `docs/session-logs/2026-04-22-pm-enforcement-branding-schema.md` para detalle.
<!-- SPECKIT END -->
