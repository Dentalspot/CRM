...
**Plan activo**: [specs/030-treatment-budget-progress/plan.md](specs/030-treatment-budget-progress/plan.md)

**Active feature**: `030-treatment-budget-progress` **PLANNING — Phase 1 complete, listo para `/speckit-tasks`** (2026-06-03). Unificar flow clínico-económico de DentalSpot: presupuesto desde odontograma → PostSession lista items pending → tilda lo hecho → cobro contextual con monto pre-rellenado → balance actualizado en `v_budget_balance`. Scope MVP: Bloques 1+2+5 (items con estado + PostSession enriquecido + polish). Bloques 3 (vista paciente con barra de progreso) y 4 (split notas público/privado) deferidos a sesión siguiente. Gracias al Bloque 0, scope reducido: `treatment_budgets`, `treatment_budget_items`, `v_budget_balance` YA EXISTEN; `patient_payments.budget_id` Y `.appointment_id` YA EXISTEN; cero migration extra para vínculos. Migration única `20260604000001_budget_items_status_and_audit.sql`: 3 columnas nuevas (`status`, `completed_at`, `completed_in_appointment_id`) + 2 índices parciales + trigger BEFORE UPDATE `check_budget_item_revert` (FR-016 autoridad) + 2 RLS policies UPDATE (`tbi_dentist_update` + `tbi_admin_update`, NO assistant) + ampliación CHECK `clinical_audit_log.resource_type` con `budget_item` + `payment`. Frontend: nuevo `src/lib/api/budgetApi.js` (~150 líneas) + `src/features/odontogram/api/budgetSyncApi.js` (~80 líneas) + `BudgetItemsChecklistStep.jsx` (~200 líneas) + EDIT `PostSessionModal.jsx` (insertar step `items` entre `document` y `payment`) + EDIT `Odontogram.jsx` (auto-crea item al marcar tratamiento) + EXTEND `registerSessionPayment` con `budgetId` opcional. Estimado 9-12h. Spec + plan + research (12 items) + data-model + contracts (migration SQL + frontend components) + quickstart con 10 smokes generados. Constitution 6/6 PASS. Smoke driver: Cristobal Tagle + Solange Vulasich en Los Álamos con endodoncia + limpieza + obturación, cierra cita tildando 2, cobro $105.000 efectivo, verifica balance_due = $30.000 en `v_budget_balance`.

**Previous feature**: `028-appointment-dentist-assignment` **IMPLEMENTED + DEPLOYED + SMOKE-TESTED** (2026-05-29). Asignar dentista responsable obligatorio a cada cita en el modal de agendamiento. Driver: Odontología Los Álamos sumó al Dr. Pablo Ceballos (ortodoncia) junto a Cristobal Tagle, ambos atendiendo en box 1, sin forma formal de declarar quién atiende qué cita. Reusa `appointments.therapist_id` existente (NO nueva columna). Migration única: trigger valida dentista activo de la org + `WITH CHECK` en RLS `appt_dentist_update` + ampliación de CHECK constraint en `clinical_audit_log` (action: `view`, `create`, `update`, `cancel`, `appointment_reassigned`; resource_type: `appointment`) — cierra incidentalmente bug pre-existente de audit logs del módulo asistente que caían silenciosamente. Frontend: refactor de `OrgCalendarView` para soportar default "Todos los dentistas" con persistencia URL (`?dentist=uuid`), helper nuevo `getDentistColor`, border-l-4 + footer Dr. {apellido} en chips, hook `useUserRoleInOrg` para default smart por rol. Aplica a vistas asistente + clinic_admin; el dentista puro mantiene su `AppointmentModal.jsx` con default = self. Out of scope: booking online paciente, `clinic_box_dentists`, push notifications. Spec + plan + research + data-model + contracts (migration SQL + frontend components) + quickstart con 10 smokes generados. Listo para `/speckit-tasks`.

**Previous feature**: `027-patient-dentist-landings` PLANNING (2026-05-26). Restructurar home en 2 landings separadas por audiencia: paciente (`/`) y dentista (`/para-dentistas`). Patrón Airbnb. Fase 1: estructura text-only + analytics GA4 nuevo + Meta Pixel existente. Sin PHI, sin cambios DB. Spec, plan, research, data-model, contracts/analytics-events y quickstart en `specs/027-patient-dentist-landings/`.
