...
**Active feature**: `020-migrate-mp-brand` — Plan completo 2026-04-22. Rebrand de 3 edge functions MP (FonoKit → DentalSpot), 14 string replacements, 0 migraciones, 0 src/** edits. Pre-requisites operacionales **✅ completados** (app MP Dentalspot creada, Access Token sandbox, webhook + signing key, "Guardar" clickeado). Estado actual: post `/speckit-plan`, pre `/speckit-tasks` o `/speckit-implement`. Ver [plan.md](specs/020-migrate-mp-brand/plan.md) + [quickstart.md](specs/020-migrate-mp-brand/quickstart.md). F-014 fix (commit 3593b12) preservación crítica vía FR-011/FR-012 + smoke Test 5. Meta-spec `fix-mercadopago-critical-bugs` diferido post-rebrand.
<!-- SPECKIT END -->
