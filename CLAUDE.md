...
**Active feature**: — pre-launch beta (2026-04-22 PM). **Spec 022 MVP Lean DONE + Phase E DONE + Launch beta prep DONE** — App funcional para cobros con enforcement de límites activo en UI + kit de launch beta listo en `docs/launch-beta/`. Phases A+B+C+D+E+G completas. Phase E cerrada en sesión PM: hook `useActivePlanLimits` + guards en PatientModal, NewAppointmentForm, InviteTherapistModal que invocan RPC `check_plan_limit` y muestran `UpgradeModal` al exceder límite. Además cleanup branding FonoKit visible residual (5b) + regeneración `supabase/schema.sql` a PG 17.9 (server upgrade 15.8→17.6). Siguiente paso: **ejecutar preflight checklist** (ver `docs/launch-beta/checklist-preflight.md`) + elegir canal soporte + invitar 5-10 dentistas usando cupón `BETA-3M-2026` (100% off, max 30 usos, 180 días, aplicable a individual/clinic_pro, max_renewals=3). Follow-ups por prioridad: (1) E3 smoke manual enforcement (30min, sección 3 del preflight), (2) completar preflight comunicación + invitar (1h), (3) meta-spec `fix-mercadopago-critical-bugs` P0 antes de scale a 100+ users (16-22h, F-001/F-002/F-003/F-005), (4) Phase F smoke E2E completo con beta users reales (1-2h), (5) `create-clinical-boxes-table-and-enforcement` cuando se diseñe UX sillones (box siempre retorna true en RPC por ahora). Ver `docs/launch-beta/README.md` para kit de launch + `docs/session-logs/2026-04-22-pm-enforcement-branding-schema.md` para detalle de la sesión PM.
<!-- SPECKIT END -->
