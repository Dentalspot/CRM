...
**Active feature**: `022-add-plans-tier-model` — **Phase A+B DONE** (DB layer aplicado 2026-04-22), **Phase C-G PENDING** (~8-11h en 2 sesiones). Migration `20260422000001_add_plans_tier_model.sql` aplicada: schema extendido (9 columnas nuevas en 3 tablas) + 4 planes seed ($0/$14.990/$24.990/$39.990) + cupón BETA-3M-2026 + RPC `check_plan_limit` enforcement server-side. V1-V5 verification PASS. **Box enforcement diferido** (tabla no existe → follow-up spec `create-clinical-boxes-table-and-enforcement`). Próximas sesiones: Phase C edge functions (billing_cycle + renewal counter, F-014 preservation crítica), Phase D frontend MembershipPlansPage rebuild, Phase E enforcement integration (hook + 4 flows), Phase F smoke tests E2E, Phase G close. Ver [plan.md §Phase C](specs/022-add-plans-tier-model/plan.md) + [quickstart.md §Phase C](specs/022-add-plans-tier-model/quickstart.md) para pickup.
<!-- SPECKIT END -->
